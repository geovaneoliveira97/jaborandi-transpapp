...
export default function StopsList({ stops, lineColor }: StopsListProps) {
  ...
  if (stops.length === 0) return (
    <div className={`${card} p-4 text-center`}>
      <p className={`text-sm ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>Informações de trajeto não disponíveis.</p>
    </div>
  )
          ...
          <li key={stop} className="flex items-start gap-3">
            <div className="flex flex-col items-center shrink-0" aria-hidden="true">
              <div className="w-2.5 h-2.5 rounded-full border-2 mt-1"
                style={{ borderColor: lineColor, backgroundColor: i === 0 || i === stops.length - 1 ? lineColor : 'transparent' }} />
              {i < stops.length - 1 && <div className="w-0.5 h-6 mt-0.5" style={{ backgroundColor: lineColor + '40' }} />}
            </div>
            <p className={`text-sm pb-3 ${i === 0 || i === stops.length - 1 ? isDark ? 'text-white font-semibold' : 'text-gray-900 font-semibold' : isDark ? 'text-gray-400' : 'text-gray-500'}`}>
              {stop}
            </p>
          </li>
        ))}
